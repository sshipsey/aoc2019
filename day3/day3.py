...
def part1(wire1, wire2):
  ...
  while (ptr1 < len(wire1)  and ptr2 < len(wire2)):
    ...
    if (ptr1 < len(wire1)):
      if (wire1[ptr1][0] == "R"):
        x1 += 1
        wire1History.append([x1, y1])
        wire1[ptr1] = "R" + str(int(wire1[ptr1][1:]) - 1)
      if (wire1[ptr1][0] == "D"):
        y1 += 1
        wire1History.append([x1, y1])
        wire1[ptr1] = "D" + str(int(wire1[ptr1][1:]) - 1)
      if (wire1[ptr1][0] == "L"):
        x1 -= 1
        wire1History.append([x1, y1])
        wire1[ptr1] = "L" + str(int(wire1[ptr1][1:]) - 1)
      if (wire1[ptr1][0] == "U"):
        y1 -= 1
        wire1History.append([x1, y1])
        wire1[ptr1] = "U" + str(int(wire1[ptr1][1:]) - 1)

    # Wire 2
    if (ptr2 < len(wire2)):
      if (wire2[ptr2][0] == "R"):
        x2 += 1
        wire2History.append([x2, y2])
        wire2[ptr2] = "R" + str(int(wire2[ptr2][1:]) - 1)
      if (wire2[ptr2][0] == "D"):
        y2 += 1
        wire2History.append([x2, y2])
        wire2[ptr2] = "D" + str(int(wire2[ptr2][1:]) - 1)
      if (wire2[ptr2][0] == "L"):
        x2 -= 1
        wire2History.append([x2, y2])
        wire2[ptr2] = "L" + str(int(wire2[ptr2][1:]) - 1)
      if (wire2[ptr2][0] == "U"):
        y2 -= 1
        wire2History.append([x2, y2])
        wire2[ptr2] = "U" + str(int(wire2[ptr2][1:]) - 1)

  print("we out?")
  return min([manhattanDistance(x) for x in [coord for coord in wire1History if coord in wire2History].append([coord for coord in wire2History if coord in wire1History])])
...
def part2(wire1, wire2):
  ...
  while (ptr1 < len(wire1)):
    # Wire 1

    if (wire1[ptr1][0] == "R"):
      x1 += 1
      wire1History[(x1, y1)] = len1
      wire1[ptr1] = "R" + str(int(wire1[ptr1][1:]) - 1)
    if (wire1[ptr1][0] == "D"):
      y1 += 1
      wire1History[(x1, y1)] = len1
  ...
  while (ptr2 < len(wire2)):

    # Wire 2
    if (wire2[ptr2][0] == "R"):
      x2 += 1
      wire2History[(x2, y2)] = len2
      wire2[ptr2] = "R" + str(int(wire2[ptr2][1:]) - 1)
    if (wire2[ptr2][0] == "D"):
      y2 += 1
      wire2History[(x2, y2)] = len2
